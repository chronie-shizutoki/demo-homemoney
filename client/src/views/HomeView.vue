    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!jsonData || jsonData.length === 0) {
          ElMessage.error(t('import.noDataFound'));
          return;
        }
        ...
        for (const item of jsonData) {
          // 尝试不同的列名格式，确保兼容性
          const expenseData = {
            type: item[t('expense.columns.type')] || item['类型'] || '',
            amount: parseFloat(item[t('expense.columns.amount')] || item['金额'] || 0),
            remark: item[t('expense.columns.remark')] || item['备注'] || '',
            time: item[t('expense.columns.date')] || item['日期'] || new Date().toISOString().split('T')[0]
          };
          
          // 验证数据有效性
          if (expenseData.type && !isNaN(expenseData.amount) && expenseData.amount > 0) {
        ...
        if (importedCount > 0) {
          ElMessage.success(`${t('import.success')} ${importedCount} ${t('import.recordsImported')}`);
        } else {
          ElMessage.warning(t('import.noValidRecords'));
        }
      } catch (error) {
...
const loadCsvExpenses = async () => {
  ...
  try {
    // 从浏览器数据库获取所有数据
    const parsedData = await getExpenses();
