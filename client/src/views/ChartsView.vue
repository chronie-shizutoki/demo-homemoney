...
const loadCsvExpenses = async () => {
  ...
  try {
    ...
    if (csvExpenses.value.length === 0) {
      console.warn('ChartsView: No valid data found in browser database');
    } else {
      console.log('ChartsView: Data loaded from browser database, count:', csvExpenses.value.length);
    }
  } catch (err) {
