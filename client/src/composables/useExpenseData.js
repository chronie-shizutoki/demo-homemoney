...
export function useExpenseData () {
  ...
  const fetchData = async (forceRefresh = false) => {
    ...
    try {
      ...
      // 确保 newData 是一个数组，即使 browserDB 返回 null 或 undefined
      const expensesData = Array.isArray(newData) ? newData : [];

      // 当强制刷新时直接更新数据，否则检查内容变化
      if (forceRefresh || JSON.stringify(expenses.value) !== JSON.stringify(expensesData)) {
