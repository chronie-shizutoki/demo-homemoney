...
import { addExpense } from '@/utils/browserDB';
...
const submitCustomEntries = async () => {
  try {
    ...
    ElMessage.error(error.message || t('expense.addFailed'));
  } finally {
    loading.value = false;
  }
};
...
// 添加消费记录
const addExpenseFromContent = async () => {
  ...
  try {
    loading.value = true;
    // 这里简化处理，实际应用中可能需要解析content字符串
    // 例如："食品 面包 15.5 2023-09-10"
    const lines = form.value.content.trim().split('\n');
    const count = lines.length;
    
    // 为每条记录创建一个消费对象
    for (const line of lines) {
      // 这里只是简单处理，实际应用中可能需要更复杂的解析逻辑
      const expense = {
        type: '其他', // 默认类型
        remark: line.trim(),
        amount: 0, // 默认金额
    ...
    ElMessage.success(t('expense.addSuccess', { count }));
    cancelAdd();
  } catch (error) {
    console.error('添加失败:', error);
    ElMessage.error(error.message || t('expense.addFailed'));
